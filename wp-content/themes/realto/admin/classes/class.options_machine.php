	...
	public static function optionsframework_machine($options) {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				//border option
				case 'of_property_features':
						
					
					$property_stored = $smof_data[$value['id']];
					
					
					$output .= '<div class="myinput_wrap">';
					$output .= '<input type="text" class="of-input mini2" name="'.$value['id'].'[of_feature_title]" id="'. $value['id'] .'_of_feature_title" value="'. $property_stored["of_feature_title"] .'" />';
					$output .= '</div>';
					
					/* Border Style */
					$output .= '<div class="select_wrapper border-style">';
					$output .= '<select class="of-border of-border-style select" name="'.$value['id'].'[of_feature_area]" id="'. $value['id'].'_of_feature_area">';
					...
					foreach ($areas as $i=>$area){
						$output .= '<option value="'. $i .'" ' . selected($property_stored['of_feature_area'], $i, false) . '>'. $area .'</option>';		
					}

...
include_once(NT_FUNCTIONS.'/tgm-plugin-activation/class-tgm-plugin-activation.php'); // Plugin Activation Class
include_once(NT_FUNCTIONS.'/tgm-plugin-activation/tgm-plugin-activator.php'); // Plugin Activator 
...
require_once(NT_FUNCTIONS.'/widgets/nt-flickr-feeds.php'); // footer flickr feeds
...
function listing_price() {
	...
	if($price_meta!=""):
		if(get_post_meta(get_the_ID(), 'nt_status', true)=="for-rent"){
			...
			$price_meta = $nt_option["site_currency"]."".$price_meta."/".get_post_meta(get_the_ID(), 'nt_period', true);
		...
		if(get_post_meta(get_the_ID(), 'nt_status', true)!="inquire"):
			if($nt_option["listing_price_prefix"]!=""):
				$prefix = $nt_option["listing_price_prefix"];
...
function nt_property_pic_list(){?>
	...
	<a class="span4 overlay" href="<?php the_permalink();?>" title="<?php the_title();?>">
			...
			'<li ' . $class . '>' .
				$args->before .
					'<a href="' . home_url( '/' ) . '" title="Home">' .
						$args->link_before . '<i class="icon-home"></i>' . $args->link_after .
					'</a>' .
				$args->after .
			'</li>';
...
function reato_property_features(){
   ...
   <?php if($nt_option["sh_plotsize"]==1):?>
    <li class="info-label clearfix">
   ...
   <?php if($nt_option["sh_livingarea"]==1):?>
    <li class="info-label clearfix">
...
?>	
    ...
    <input type="hidden" name="post_type" value="property" />
       <?php if($nt_option["sp_keyword"]=="1"):?> 
        <div class="row-fluid">
            <div class="span10">
                <input type="text" class="input-block-level" name="search_keyword" value="" placeholder="<?php if(isset($nt_option["s_propertykeyword"])){ echo $nt_option["s_propertykeyword"];} ?>" />
            </div>
        ...
        <div class="row-fluid">
         ...
         <?php endif; ?>
          ...
          <?php if($nt_option["sp_beds"]=="1"):?>  
           ...
           <?php if($nt_option["sp_baths"]=="1"):?>
            <div class="span1">
                <select class="input-block-level select" name="baths">
                    <option value=""><?php if(isset($nt_option["s_bath"])){ echo $nt_option["s_bath"];} ?></option>
                    <option value="1">1+ <?php if(isset($nt_option["s_bath"])){ echo $nt_option["s_bath"];} ?></option>
                    <option value="2">2+ <?php if(isset($nt_option["s_bath"])){ echo $nt_option["s_bath"];} ?></option>
                    <option value="3">3+ <?php if(isset($nt_option["s_bath"])){ echo $nt_option["s_bath"];} ?></option>
           ...
           <?php if($nt_option["sp_min_price"]=="1"):?>
            <div class="span2">
                <input class="input-block-level select" type="text" name="min-price" value="" placeholder="<?php if(isset($nt_option["s_minprice"])){echo $nt_option["site_currency"]." ".$nt_option["s_minprice"];}?>">
            </div>
           <?php endif; ?>
           
           <?php if($nt_option["sp_min_price"]=="1"):?> 
            <div class="span2">
                <input class="input-block-level select" type="text" name="max-price" value="" placeholder="<?php if(isset($nt_option["s_maxprice"])){echo $nt_option["site_currency"]." ".$nt_option["s_maxprice"];}?>">
            </div>
            <?php endif; ?>
